...
- State:
  ...
  - placeableEntities: { label, value, icon, group }[] — what can be put down on a canvas: creatableEntities, then WE's own blocks that have a form but are left out of general pickers (`offered: false` — Image, Text, Video, Audio, File), grouped 'Blocks'. Use it for a canvas's 'add' chooser; use creatableEntities for 'create something' elsewhere
