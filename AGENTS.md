...
- **Neighbourhood** — a _shared_ perspective, synced peer-to-peer. A shared Space is a neighbourhood.
...
- **Space** — a community/context, backed by an AD4M perspective. _Personal_ (local) or _shared_
...
| `@we/module-globe` · `-call` · `-notes` · `-transcribe` · `-graph`    | module-system/*                | Bundled feature modules; globe is a _family_ (module · protocol · layers · widget)                                                                              | Agnostic (components injected)           |
...
1. **Primitives are the framework-neutral currency.** `@we/primitives` are Lit web components, so
   ...
   per-framework _declarations_ are generated from the Custom Elements Manifest for hand-authored
...
**A template is one of two things.** A _shell_ owns a space's chrome, arrangement and route table;
a _view_ renders one section inside one. `meta.role` says which, and absent means shell. A shell
...
what is a fact about the _type_; reify what is a claim about a _pair_.
...
| Change what one section of a space _is_                    | **View**                                      |
...
- **Component vs fragment.** `AvatarStack` is a component (overlap maths); the count beside it is a
  ...
  fragment. The question is never how complicated it is — it is whether it must _do_ something or
  merely be _arranged_.
- **Fragment vs operator.** `field` stayed a fragment because _which event carries the value_ is
...
words a person reads at install time; anything not granted is _absent_ rather than blocked, leaving
...
a _second_ one, and reactivity silently stops crossing the boundary. Blocks and components have a
...
Declare one when a _part_ of your layout should adapt to itself — a two-pane workspace whose right
...
resource inside it. For different _values_ — padding, gap, width, font size — use `*UpProps`, which
...
this drag mean" is never the handle's business: the editor's panel rails grow _leftwards_ from a
...
A zone _is_ the container and its children _are_ the items, which is what makes nesting free: a
...
and nothing else. What that _means_ is the consumer's business, and it differs: a kanban route
...
Because nesting here is expressed _in the DOM_, that check is `dragged.contains(zone)` — correct
...
- `board` — A container's contents at the positions somebody put them. Membership is ordinary containment, so a card composed onto the board is found like any child; position comes from Placement records parented to the same board, which is why the same note can sit on two boards in two places. Pair with layout: manual and drag-node { pin: true }, and persist a drop through recordStore.placeOnBoard. Loads nothing until a board is chosen.
  ...
  - contains: string[] — Types the board may hold beyond whatever its placements name — one query each. Defaults to the block vocabulary; anything _placed_ is loaded whether or not it is listed.
...
A scale position says _which grey_. A role says _what the colour is for_, and that is what a theme
...
is the accent _used as_ text, and `on-accent` is the text _placed on_ the accent. They are
...
colour is a _palette_ rather than a meaning — a graph's node colours by category, a chart series,
...
| `control-surface`                                        | The filled neutral of a _control_ — a slider or switch track, a progress trough, a scrollbar thumb, a secondary button, a count chip. Not a surface and not a state.                                                                                                                                                        |
...
| `surface-inverse`                                        | A surface deliberately opposite to the page — a tooltip. Holds a fixed lightness, so it does _not_ flip with the theme.                                                                                                                                                                                                     |
...
| transition     | string                                            | CSS transition. Durations may be animation tokens (`'0'`–`'500'`): `'width 300 ease-in-out'`. Prefer the token — a theme's animationSpeed preset overrides those, so `300` respects a reduced-motion setting where `300ms` overrides it. Use for a property whose _value_ changes in place (a width bound to `$local`); for something appearing and disappearing use `$if`/`$animate` transitions instead |
...
what a theme _decides_ a faint foreground is, and the contrast corrections at apply time skip it
      ...
      "children": [
        ...
        { "type": "we-text", "children": ["This cannot be undone."] },
        {
          ...
          "children": [
            {
              "type": "we-button",
              "props": { "variant": "ghost", "onClick": { "$setLocal": "confirmDeleteOpen", "value": false } },
              "children": ["Cancel"]
            },
            {
              "type": "we-button",
...
long form, a settings editor) scrolls its _content_, never its own title or its action buttons.
...
`BlockComposer` is **pull-based**. Its `onSave` does _not_ fire when the user types or when a modal
  ...
  "children": [
    ...
    },
    {
      ...
      "children": [
        {
          "type": "we-number",
          "props": { "value": { "$count": { "items": { "$store": "spaceStore.members" } } }, "shorten": true }
        },
        {
          "type": "we-text",
          "children": [
...
"routes": [
{ "path": "/_", "type": "we-text", "children": ["Post not found"] },
...
- **Changes** — one entry per file or logical group, explaining _why_ the change was
...
_tile_ layer, so the mesh and presence are untouched and this says nothing about signalling — it is
...
`slots` and store at _definition_ time so a production build contains no node and no callable
...
- _"I changed the source and the app is unchanged."_ The package ships a `dist` and it was not
...
- _"The build says it failed but the error names a package I did not touch."_ A dependency's types
...
- `packages/design-system/2-themes/THEME_AUTHORING.md` — **read this before writing or editing a
  ...
  roles pinned only where the parametric default is wrong. Several roles are _derived at apply time_
