...
## Open Questions
- **Validation:** Should `$localState` support a `validate` field (regex, min/max, required)? Or handle validation in actions?
...
## Scope
This PR would add the core `$localState` / `$local` / `$setLocal` system. Validation, derived state, and nested scopes can follow incrementally.
