...
- **WE workaround:** `@HasMany({ through: 'we://children' })` as `string[]`, manual type resolution via block registry in `loadBlocks()`. The plan document for that work was removed once it shipped; the code is `packages/block-system/shared/src/registry.ts` and `@we/backend-ad4m`'s generated `CollectionBlock`.
