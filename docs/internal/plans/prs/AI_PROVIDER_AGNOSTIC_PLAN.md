...
| ----------------------- | ------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| API key field           | `packages/entities/src/entities/AgentSettings.ts:19-20`                        | `claudeApiKey: string`, AD4M `@Property` (`we://claude_api_key`) — name is provider-specific                                                               |
...
- **Model picker**: hardcode a short list per provider (Anthropic model names are stable/known; OpenAI-compatible endpoints vary wildly), or free-text input? Free-text is simpler and matches how AD4M's own `addRecord` already works.
