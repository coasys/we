...
function record(name: string, samples: number[], stats: any, transport: string) {
  results.push({ name, spec: '03-custom-getters', transport, samples, stats });
}
...
describe('Custom SPARQL getters @ 100k links', () => {
  test('1. Channel.allItems() — instance method via SDK', async () => {
    ...
    record('Channel.allItems()', samples, stats, executor.client.transport);
  });
  ...
  test('2. Channel.totalItemCount() — instance method via SDK', async () => {
    ...
    record('Channel.totalItemCount()', samples, stats, executor.client.transport);
  });
  ...
  test('3. Channel.unprocessedItems() — set-difference via SDK', async () => {
    ...
    record('Channel.unprocessedItems()', samples, stats, executor.client.transport);
  });
  ...
  test('4. ConversationSubgroup.itemsData() — instance method via SDK', async () => {
    ...
    record('Subgroup.itemsData()', samples, stats, executor.client.transport);
  });
  ...
  test('5. Message.replyingTo — getter evaluated via ORM .get()', async () => {
    ...
    record('Message.get() with replyingTo getter', samples, stats, executor.client.transport);
  });
  ...
  test('6. Conversation.stats() — custom method (SPARQL + get)', async () => {
    ...
    record('Conversation.stats()', samples, stats, executor.client.transport);
  });
  ...
  test('7. ConversationSubgroup.stats() — custom method', async () => {
    ...
    record('Subgroup.stats()', samples, stats, executor.client.transport);
  });
});
