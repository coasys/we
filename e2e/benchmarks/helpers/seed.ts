 ...
 */
import { LinkInput, ApiClient } from './api-client';
import { createRng, generateId } from './stats';
import {
  ENTRY_TYPE,
  BODY,
  CHANNEL_NAME,
...
export const DEFAULT_SCALE: ScaleConfig = {
  ...
  reactionsPerMessage: 3, // applied to ~50% of messages
...
/** Estimate total link count for a config. */
export function estimateLinkCount(cfg: ScaleConfig): number {
  ...
  // Per entity: entry_type link + parent→child link + property links
  // Channel: entry_type + name + description + is_conversation = 4
  // Message: entry_type + body + parent_link = 3
  // Conversation: entry_type + parent_link = 2
  // Subgroup: entry_type + parent_link = 2
  // Subgroup messages: entry_type + body + subgroup_item link = 3

  const channelLinks = cfg.numChannels * 4;
  const messageLinks = msgs * 3; // entry_type + body + channel→message
...
export function generateSeedData(cfg: ScaleConfig = DEFAULT_SCALE, seed = 42): SeedManifest {
  ...
  for (let ch = 0; ch < cfg.numChannels; ch++) {
    ...
    // Replies — random message→message links
    ...
    for (let t = 0; t < cfg.threadsPerChannel && channelMessageIds.length > 0; t++) {
      ...
      for (let d = 0; d < cfg.threadDepth; d++) {
        ...
        // Next depth uses this message as parent (chain)
    ...
    for (let c = 0; c < cfg.conversationsPerChannel; c++) {
      ...
      for (let sg = 0; sg < cfg.subgroupsPerConversation; sg++) {
        ...
        // Messages in subgroup (reference existing channel messages when possible)
        for (let sm = 0; sm < cfg.messagesPerSubgroup; sm++) {
          let itemId: string;
          if (channelMessageIds.length > 0 && rng() < 0.7) {
            // Re-use an existing message ~70% of the time
...
/**
 * Seed an executor perspective with generated links.
 * Returns the manifest for use in benchmark queries.
 */
