...
async function main() {
  ...
  let fileA: string;
  let fileB: string;
  let threshold = 10; // % change to highlight
  ...
  if (nonFlags.length >= 2) {
    ...
    // Auto-detect: compare last two reports
  ...
  for (const r of current.results) {
    ...
    if (b) {
      ...
      console.log(
        `| ${r.name} | ${fmt(b.stats.mean)} | ${fmt(r.stats.mean)} | ${meanChange} | ${p95Change} |`,
      );
    } else {
      console.log(`| ${r.name} | — | ${fmt(r.stats.mean)} | NEW | — |`);
    }
  }
  ...
  for (const [key, b] of baselineMap) {
    ...
    if (!exists) {
      console.log(`| ~~${b.name}~~ | ${fmt(b.stats.mean)} | — | REMOVED | — |`);
    }
  }
  ...
  if (regressions.length > 0) {
    ...
    for (const r of regressions) {
      ...
      console.log(`   - ${r.name}: ${fmt(b.stats.mean)} → ${fmt(r.stats.mean)} (+${pct}%)`);
    }
  }
