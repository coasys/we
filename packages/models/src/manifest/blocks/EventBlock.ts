...
export const EventBlock: CoreEntityDef = {
  ...
  entity: {
    ...
    properties: {
       ...
       */
      occurrence: { type: "string", predicate: "we://occurrence", interpretationHint: "A dedup key, not a display value: the title and the start date joined, e.g. \"Design review 2026-08-20\". Always set it when you create an event. Reuse an existing event's exact value only when this is the same occasion on the same day.", identity: true, default: "" },
      title: { type: "string", predicate: "we://title", required: true, interpretationHint: "What the occasion is called, e.g. \"Design review\". No trailing period. Never include the bracketed timestamp that starts each turn — it is metadata, not speech.", default: "" },
      description: { type: "string", predicate: "we://description", interpretationHint: "What it is for, if said. Omit rather than restate the title.", default: "" },
      startDate: { type: "string", predicate: "we://start_date", required: true, interpretationHint: "Start as YYYY-MM-DDTHH:mm (local time, no timezone suffix). Resolve relative dates like \"next Tuesday at 3\", \"this weekend\" or \"on Friday\" against the bracketed timestamp leading that turn — pick the nearest matching future date. When only a day was said, use T00:00 and set allDay true. Required, so give your best resolution rather than omitting the event.", default: "" },
      endDate: { type: "string", predicate: "we://end_date", interpretationHint: "End as YYYY-MM-DDTHH:mm. Omit unless a duration or end time was actually stated.", default: "" },
      location: { type: "string", predicate: "we://location", interpretationHint: "Where it happens — a place or a link, as said. Omit if unstated.", default: "" },
      allDay: { type: "boolean", predicate: "we://all_day", interpretationHint: "True whenever only a day was said and no time of day — the common case in speech.", default: false },
