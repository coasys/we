...
its *contrast*: a narrow span is a soft theme, a full 0–100 span is a stark one. A ceiling above
...
Everything else is a *role* — `surface`, `textMuted`, `accent`, `dangerText`. Each has a parametric
...
`channels` states its own accent because a designed theme should say what its accent *is*. But pin
...
**Contrast is measured twice.** WCAG 2 *and* APCA, with the stricter governing. WCAG 2 is generous
