...
export const runtimeSettings: SchemaNode = {
  type: '$if',
  props: {
    condition: { $store: 'runtimeStore.canAdminister' },
    then: {
      type: 'Column',
      props: { gap: '600' },
      $localState: {
      ...
      children: [
        {
          type: 'Row',
          props: { gap: '300', ay: 'center', mt: '400' },
          children: [
            { type: 'we-icon', props: { name: 'lightning', size: 'md' } },
            { type: 'we-text', props: { variant: 'heading-sm' }, children: ['Data layer'] },
          ],
        },
        {
          type: 'we-text',
          ...
          children: [
            'Settings for the backend running behind this app — who it trusts, how it reaches peers, and which apps hold credentials for your agent.',
          ],
        },

        // One shared error line. Every runtime action routes through the same slot, so a failure
        // is visible wherever it happened rather than only where someone remembered to render it.
        {
          type: '$if',
          props: {
