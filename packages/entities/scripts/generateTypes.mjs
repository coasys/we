 ...
 * `@we/backend-ad4m`). Run via tsx: `pnpm --filter @we/entities generate:types`.
 */
...
function emitTypes(defs) {
  ...
  L.push(' *');
  L.push(' * Rebuild with `pnpm --filter @we/entities generate:types`.');
  L.push(' */');
  L.push("import type { RecordInstance, WeNodeRecord } from './base';");
  L.push('');
  L.push('export type { RecordInstance, WeNodeRecord };');
  L.push('');
  // Named aliases for the closed vocabularies, exported from the package root — consumers import
  // SignalMode from '@we/entities' exactly as they did when the class file declared it.
  ...
  for (const [name, def] of Object.entries(defs)) {
    const e = def.entity;
    L.push(`export interface ${name}Record extends ${def.base === 'WeNode' ? 'WeNodeRecord' : 'RecordInstance'} {`);
    ...
    for (const [rname, spec] of Object.entries(e.relations)) {
      ...
      if (spec.cardinality === 'one') L.push(`  ${rname}?: ${spec.target ? `${spec.target}Record` : 'string'};`);
      else L.push(`  ${rname}: ${def.typedArrays?.includes(rname) ? `${spec.target}Record[]` : 'string[]'};`);
    }
    for (const [rname, spec] of Object.entries(e.relations)) {
