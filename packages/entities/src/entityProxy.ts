 ...
 */
import { type EntityClass, getEntity } from './entityRegistry';
 ...
 * ```
 */
export function defineEntity(name: string): EntityClass {
  const resolve = (): EntityClass => {
    try {
  ...
  return new Proxy(function entityStandIn() {} as unknown as EntityClass, {
