 ...
 * task.
 */
export const Involvement: CoreEntityDef = {
  base: 'Ad4mModel',
  entity: {
    flag: { predicate: 'we://flag', value: 'we://involvement' },
    properties: {
      /**
       * Who, as a DID.
       *
       ...
       */
      agent: { type: 'string', predicate: 'we://involved_agent', required: true, default: '' },
      /**
       ...
       */
      kind: { type: 'string', predicate: 'we://involvement_kind', required: true, default: '' },
      /** Why, when there is something worth saying — "only for the first hour", "the API half". */
      note: { type: 'string', predicate: 'we://description', control: 'textarea', default: '' },
    },
    relations: {
      /*
        What they are involved in — any record. Untyped, so polymorphic by default, which is what
        lets a task, an event and a community's own `Sighting` all carry people through this one
        entity.
      */
      node: { target: '', cardinality: 'one', predicate: 'we://involved_in' },
    },
  },
};
