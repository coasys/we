import type { EntityEntry, EntityFieldEntry, EntityRelationEntry } from '../types.js';
 ...
 * were the source of truth, and backwards once they became artifacts of `@we/entities`' manifest.
 ...
 */
export async function extractEntities(_modelsDir?: string): Promise<EntityEntry[]> {
  const { CORE_DEFS, WE_NODE_ENTITY } = (await import('@we/entities/manifest')) as unknown as {
    CORE_DEFS: Record<string, CoreDefLike>;
    WE_NODE_ENTITY: EntityLike;
  };

  const entries: EntityEntry[] = Object.entries(CORE_DEFS).map(([name, def]) => ({
    name,
    className: name,
    extends: def.base,
...
function fieldEntries(def: CoreDefLike): EntityFieldEntry[] {
...
function relationEntries(relations: Record<string, RelationLike>): EntityRelationEntry[] {
