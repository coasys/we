...
import { AssistantThreadList } from '../src/components/AssistantThreadList';
...
function renderList(store = makeMockStore()) {
  return render(() => (
    <AssistantContext.Provider value={store}>{(<AssistantThreadList />) as JSX.Element}</AssistantContext.Provider>
  ));
}
