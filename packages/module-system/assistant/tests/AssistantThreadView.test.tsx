...
import { AssistantThreadView } from '../src/components/AssistantThreadView';
...
function renderView(store = makeMockStore()) {
  return render(() => (
    <AssistantContext.Provider value={store}>{(<AssistantThreadView />) as JSX.Element}</AssistantContext.Provider>
  ));
}
