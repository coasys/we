...
// A type alias, not an interface — the module contract types `createStore`'s return as
// `Record<string, unknown>`, and only structural object types are assignable to it.
export type AssistantStore = {
  ...
  // --- Surface visibility ---
  // The assistant opened via `templateStore.openShellView` in its pre-module form; as a module its
  // surface is an overlay slot gated on this, and the module rail's launcher toggles it.
...
export function createAssistantStore(deps: ModuleStoreDeps): AssistantStore {
  // The handles are opaque `DatasetHandle`s at the contract; this module declares
  // `backends: ['ad4m']` (its models are decorated classes), so narrowing them to the proxy type
  // here is the declared coupling, not a leak.
  ...
  const threadPerspective = () => (deps.dataset?.() ?? deps.rootDataset?.() ?? null) as PerspectiveProxy | null;
  // Assistant configuration is personal — always the root dataset.
  const configPerspective = () => (deps.rootDataset?.() ?? null) as PerspectiveProxy | null;
  ...
  async function refreshModels(): Promise<void> {
    const conn = deps.connection?.() ?? null;
...
/**
 * Module-scoped instance, set by `createAssistantStore` when the registry activates the module.
 *
 * Bundled modules are single-instance, so components reach their store through this rather than a
 * context that would need a provider mounted somewhere in the host's tree. The context still exists
 * (above) and takes precedence when present — that is how tests and harnesses inject a mock without
 * activating the module. Hot-reload caveat: a stale instance survives an HMR of this file until the
 * module re-registers, which the registry's idempotent re-registration handles on the next boot.
 */
let instance: AssistantStore | null = null;

export function useAssistantStore(): AssistantStore {
  const ctx = useContext(AssistantContext);
  if (ctx) return ctx;
  if (!instance) throw new Error('assistant module not activated — is it in the seed and registered?');
  return instance;
}
