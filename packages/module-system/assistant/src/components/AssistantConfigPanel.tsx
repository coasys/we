...
import { parseIdList, useAssistantStore } from '../store';
          ...
          {(t) => (
            ...
            >
              ...
              <we-text
                ...
                color={tab() === t.id ? 'neutral-900' : 'neutral-600'}
              >
                {t.label}
              </we-text>
            </Row>
            ...
            <Column gap="250">
              <Field label="Name">
                <we-input
                  ...
                  on:input={(e: CustomEvent) => setName(e.detail as string)}
                />
              </Field>
...
function GrantGroup(props: {
  ...
  const grantedSet = () =>
    new Set(parseIdList(store.assistants().find((a) => a.id === props.assistant.id)?.[props.field]));
  ...
  return (
    <Column gap="150">
      ...
      <Show
        ...
        fallback={<we-text fontSize="200" color="neutral-400">{`No ${props.title.toLowerCase()} defined`}</we-text>}
      >
        <Column gap="50">
              ...
              return (
                ...
                >
                  <we-icon
                    ...
                    size="sm"
                    color={on() ? 'primary-500' : 'neutral-400'}
                  />
    ...
    <Column gap="300">
      ...
      <EditorCard title={editingId() ? 'Edit personality' : 'New personality'}>
        ...
        <we-textarea
          ...
          rows={4}
          resize="vertical"
          placeholder="Personality guidance…"
          on:input={(e: CustomEvent) => setBody(e.detail as string)}
        />
    ...
    <Column gap="300">
      ...
      <EditorCard title={editingId() ? 'Edit skill' : 'New skill'}>
        ...
        <we-input
          ...
          on:input={(e: CustomEvent) => setDescription(e.detail as string)}
        />
        <we-textarea
          value={body()}
          rows={4}
        ...
        <Show
          ...
          fallback={
            <we-input
              ...
              on:input={(e: CustomEvent) => setUrl(e.detail as string)}
            />
          }
        >
          <we-input
            ...
            on:input={(e: CustomEvent) => setCommand(e.detail as string)}
          />
        </Show>
        <we-input
          value={auth()}
          size="sm"
          placeholder="Auth (optional JSON / token)"
          on:input={(e: CustomEvent) => setAuth(e.detail as string)}
        />
        <EditorActions editing={!!editingId()} onSubmit={submit} onCancel={reset} />
      </EditorCard>
    </Column>
