...
export async function buildExecutor(config: ExecutorConfig): Promise<string> {
  ...
  execSync(`git clone --depth 1 --branch ${branch} --single-branch "${adamRepoPath}" "${buildDir}"`, {
    stdio: 'pipe',
    timeout: 60000,
  });
  ...
  const cachePath = denoRev ? join(snapshotCacheDir(), `${denoRev}.bin`) : null;
...
export async function startExecutor(binaryPath: string, config: ExecutorConfig): Promise<ChildProcess> {
