...
import { t5TopologyTable, m1MeshToSfu, m2SfuToMesh, m4SfuOfflineFallback } from './scenarios/index.js';
import { consoleReport, jsonReport } from './reporters.js';
import { config, validateAdamRepo } from './config.js';

const ALL_SCENARIOS: Scenario[] = [t5TopologyTable, m1MeshToSfu, m2SfuToMesh, m4SfuOfflineFallback];
...
async function main(): Promise<void> {
  ...
  const scenarios =
    args.scenarios.length > 0 ? ALL_SCENARIOS.filter((s) => args.scenarios.includes(s.id)) : ALL_SCENARIOS;
  ...
  if (binaryPaths.size === 0) {
    console.error('[runner] No executors available.');
    process.exit(1);
  }
