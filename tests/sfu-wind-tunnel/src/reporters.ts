...
export function comparisonReport(allResults: Map<string, ScenarioResult[]>, outputPath: string): void {
  const lines: string[] = [];
  ...
  for (const scenarioId of scenarioIds) {
    ...
    for (const branch of branches) {
      ...
      for (const [key, value] of Object.entries(result.metrics)) {
        ...
        metricsMap.get(key)!.set(branch, typeof value === 'number' ? value.toFixed(2) : String(value));
      }
    }
