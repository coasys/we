...
async function main() {
  ...
  if (requiresExecutor) {
    ...
    if (!token) {
      console.error(
        '[run-webrtc] T/M/F/S/W5 scenarios need an admin token. ' + 'Set --executor-token or AD4M_ADMIN_TOKEN.',
      );
      process.exit(1);
    }
  ...
  for (const scenario of scenarios) {
    ...
    const ctx: ScenarioContext = {
      ...
      branch: args.label,
      port: args.executorUrl ? Number(new URL(args.executorUrl).port || 12000) : Number(process.env.AD4M_PORT || 12000),
