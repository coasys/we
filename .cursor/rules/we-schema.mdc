...
- State:
  ...
  - templateOverrideOptions: { label, value }[] — options for the per-space template override picker, with a leading "Use the space’s default" entry whose value is ''. Pre-built because a schema can $map a store array into options but cannot prepend one, and without that entry overriding is one-way
  - themeOverrideOptions: { label, value }[] — the same, for themes
  - moduleInstallSettings: { id, name, description, icon, installed }[] — every registered module paired with whether this agent wants it anywhere. The global Settings → Modules list. Its per-space counterpart is `modules` on each spaceList row, which carries enabled/installed/muted/active together
...
- Actions:
  ...
  - setModuleMuted(moduleId: string, muted: boolean, spaceUuid?): hides a module for this agent in one space, without changing what the community runs. Private: written to the root dataset, never to the space, so muting is not broadcast to other members
  - setSpaceTemplateOverride(templateId: string, spaceUuid?): sets the template THIS AGENT sees in one space, overriding the community's default. '' returns to following the space. Private, and applied immediately when that space is the one on screen
  - setSpaceThemeOverride(themeId: string, spaceUuid?): sets the theme THIS AGENT sees in one space. '' returns to following the space. Private
