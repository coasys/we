...
- State:
  ...
  - linkLanguageTemplateOptions: { label, value }[] — the link language templates a shared space can be published with, sorted by name, for a we-select; value is the template address. Only templates publishing can fill in are listed. Empty until the backend answers, and on a backend with no choice to offer
